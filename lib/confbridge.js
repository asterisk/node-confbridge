...
function ConfBridge(ari) {
  ...
  self.joinMixingBridge = function(bridge, channel) {
    ...
    bridge.on('ChannelLeftBridge', function (event, instances) {
      bridge.play({media: 'sound:confbridge-leave'}, playback, function (err, playback) {});
      var mixingBridge = instances.bridge;
      if (mixingBridge.channels.length === 0 && mixingBridge.id === bridge.id) {
        bridge.destroy(function (err) {});
      }
    });
    bridge.addChannel({channel: channel.id}, function (err) {
      bridge.play({media: 'sound:confbridge-join'}, playback, function (err, playback) {});
    });
  }
